...
public class LoginClass
{
	...
	public LoginClass(string usr, string pass)
	{
        username = usr;
        password = pass;
        connect = new SqlConnection(ConfigurationManager.ConnectionStrings["AutomartConnectionString"].ConnectionString);//prepares connection; CommunityAssist connection string for assignment
	}

    public int ValidateLogin()//returned something other than zero = valid login
    {
        //set the personID to 0
        int personID = 0;
        //create the SQL String
        string sql = "Select PersonKey, CustomerPassCode, CustomerHashedPassWord From Customer.RegisteredCustomer "
            + "Where email=@email and CustomerPassword=@password"; //might change for assignment
        ...
        while (reader.Read())
        {
            ...
            if (reader["CustomerPassCode"] != null)
            {
                ...
                Byte[] savedPass = (Byte[])reader["CustomerHashedPassword"];
